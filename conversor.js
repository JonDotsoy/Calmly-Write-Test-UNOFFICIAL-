...
function generateDocx(htmlString){ 
	...
	this.createNodeXML = function(node,xmlDoc,padreXML){
		...
		*/
		this.createMyTextNode=function (node,xmlDoc){
			...
			//else if( !node.data.split('\n').join('') )   return xmlDoc.createElement("w:Noinsert"); 
	...
	<w:rFonts w:hint="default" w:ascii="Wingdings" w:hAnsi="Wingdings"/>
		...
		/*this.getDataUri=function (url, callback) {
		    ...
		    image.onload = function () {
		        ...
		        // Get raw image data. ESTA LLAMADA SI SE QUIERE QUITAR LA EXTENSIÓN ETC
		       // callback(canvas.toDataURL('image/png').replace(/^data:image\/(png|jpg);base64,/, ''));

		        // ... or get as Data URI. ESTA LLAMAD PARA RECUPERAR TODO EL DATA IMAGE. LE INDICO EL FORMATO DEIMAGEN DESEADO = png
		...
		*/
		this.escalarIMG=function (imgwidth, imgheight){
			...
			var width_inch = imgwidth / 96; //alert('width inch ='+width_inch); 
		  	var height_inch = imgheight / 96; 

		  	var width_emu = width_inch * 914400;
		  	var height_emu = height_inch * 914400;

		  	var pgSzW =  (16 * 360000); // // ANCHO -MARGEN   // // ancho de página en EMUs
		...
		*/
		...
		/*
		  	  // Usage. 
			if(!this.stringStartsWith(dataImg, 'data:')){  alert('convertimos IMG!!!');

					// PROBAR ESTA FUNCION
					
					// LLAMADA FUNCIÓN ASÍNCRONA QUE LEE IMAGEN ORIGEN Y LA CONVIERTE A BITMAP
					getDataUri(dataImg, function(dataUri) {
					    // Do whatever you'd like with the Data URI!

					    alert('fin prueba dataUri :'+dataUri);
					    console.log('image URI = ');console.log(dataUri);
					});
  ...
  w:styleId="ListParagraph" w:type="paragraph">
		...
		numberingString = cabeceraNumbering+numberingString+numIdString+footerNumbering;
			...
			//console.log('resultado recursiva:'+XmlDocumentDocx);
	...
	this.parseHTMLtoDocx = function (node,xmlDoc,padreXML){  //console.log('dime contador de listas = '+countList);
		...
		var dataImg = node.attributes[0].value;  
				...
				if (!this.stringStartsWith(dataImg, 'data:')){  imgEle =  this.nodeVoid(xmlDoc);   }
	...
	/* INCLUIR AL FINAL ANTES DE CERRAR EL /W:BODY
<w:sectPr>
