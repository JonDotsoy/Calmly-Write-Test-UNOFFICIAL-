...
this.inputenablecheck=true;
  ...
  if (this.editor_tooltip===null) {
          ...
          _this.seleccionPrevia = window.getSelection().getRangeAt(0);
           ...
           if (enlaces.length>0) {
              ...
              if (window.getSelection() && window.getSelection().toString() && window.getSelection().toString().trim().indexOf(' ')==-1 && ValidURL(window.getSelection().toString().trim())) {
                  ...
                  by('id:linkboxinput').value(chrome.i18n.getMessage("enterurlmsg"));
              }
           }
      ...
      linkboxinput.focus(function (e) {
          if (this.value()!=chrome.i18n.getMessage("enterurlmsg") && this.value()!='') {
            this.seed.select();
          } else {
      ...
      linkboxinput.clic(function (e) {
        if (this.value()==chrome.i18n.getMessage("enterurlmsg")) {
          this.seed.setSelectionRange(0,0);
        }
      ...
      linkboxinput.keypress(function (e) {
          if (e.keyCode=='13') {
              ...
              if (this.value().replace(/^\s+|\s+$/g, '')!='' && this.value()!=chrome.i18n.getMessage("enterurlmsg")) {
  ...
  this.mainarea.paste(function () { _this.documentsaved=false; _this.documentcloudsaved=false; _this.hideTooltip(); _this.removeStyles(); _this.removeNBSP(); _this.marginImgs(); _this.interactionImgs(); make_links_openable(); trytoautosave();});
...
this.mainarea.keypress(function (e) {
 ...
 /*
     if (e.keyCode===8 || e.keyCode===46) {
           ...
           var hh=_this.mainarea.seed.children;
           if (hh.length==1 && isEmptyContent(new Grape(hh[0]))) {
           e.preventDefault();
           return false;
           }  
       ...
       if ((e.keyCode===32 && !e.metaKey && !e.ctrlKey) || e.keyCode==13) {
        ...
        if (window.getSelection().anchorNode.parentNode.tagName=='LI') padre=window.getSelection().anchorNode.parentNode;
          ...
          if (lastchar==' ') {
            ...
            /*
            if (padres.spans.length || padre.tagName=='H1' || padre.tagName=='H2' || padre.tagName=='H3') {
            document.execCommand('insertText',false,' ');
            } else {
            document.execCommand('insertHTML',false,' ');
            }
    ...
    if (isMac && e.metaKey && e.shiftKey && e.keyCode==70) {
        //mac
        by('id:opfull').clic();
    } else if (e.metaKey && (e.keyCode==109 || e.keyCode==189 || e.keyCode==45 || e.keyCode==54)) {
      _this.enableZoomOut();
      ...
      //if (e.ctrlKey || e.metaKey) {
       if (e.keyCode==109 || e.keyCode==189 || e.keyCode==45 || e.keyCode==54) {
        ...
        _this.disableZoomOut();
       } else if (e.keyCode===70 && e.shiftKey==false) {
        if (menuopen) calmlylogo.clic(); 
        _this.disableZoomOut();
        by('id:ctrlfinput').focus();
        by('id:ctrlf').animate(300).ease('out').style('top','34px');
          ...
          e.cancelBubble = true;
          _this.saveSelection();
          raceme.by('id:opsavedirect').click();
         return false
 ...
 this.mainarea.keyup(function (e) {
         ...
         if (_this.getSelectionY()>window.innerHeight-170) {
            if (scrollanimation) scrollanimation.stop();
            scrollanimation=wrapper.animate(300).ease('out').scrollTop(wrapper.scrollTop()+171);
  ...
  this.mainarea.input(function (e) {
      ...
      _this.documentcloudsaved=false;
      setTimeout(function () {
...
if (name=='formatBlock') {
  ...
  if (padre.block.tagName==args || '<'+padre.block.tagName+'>'==args) {
        ...
        if (args=='P') document.execCommand('outdent');
        args='P';
...
Editor.fn.marginImgs=function () {
  ...
  imgs.each(function () {
    ...
    if (this.attr('data-src')=='' && this.attr('src')!='null' && this.attr('src')!='img/notfound.png' && this.attr('src').replace(/^\s+|\s+$/g, '').match(/^((https?|ftp):)?\/\/.*(jpeg|jpg|png|gif|JPEG|JPG|PNG|GIF)$/) != null) {
      var uriimg=this.attr('src');
      this.attr('src','img/loader.png').style('width','60px').style('margin','auto');
      this.attr('data-src',uriimg);
      var _thisimg=this;
         ...
         format:'blob',
            ...
            _thisimg.seed.src=window.URL.createObjectURL(response);
            setTimeout(function () {
         ...
         fail:function () {
            ...
            _thisimg.seed.src="img/notfound.png";
            _this.ajustarImg(_thisimg);
            trytoautosave(); 
...
Editor.fn.ajustarImg=function (gpr) {
  ...
  if (gpr.getParent().id()=='main' || ((gpr.getParent().tagName()=='P' || this.HTMLheaders.indexOf(gpr.getParent().tagName()!=-1)) && (gpr.getParent().getParent() && gpr.getParent().getParent().id()=='main'))) { 
    if (ancho_img>ancho_screen) {
        ...
        if (gpr.seed.naturalWidth==0) {
          gpr.attr('src','img/notfound.png');
        }
    }
  } else {
...
if (nodo.nodeType==1) {
    ...
    if (nodo.childNodes.length==0) {
      ...
      if (name=='IMG') {
          ...
          if (nodo.getAttribute('src').substring(0,5)!="data:") {
            if (includeimages) {
            htmlelement+="<img src='"+getBase64Image(nodo)+"' ";
            } else {
            htmlelement+="<img src='"+nodo.getAttribute('data-src')+"' ";  
            }
